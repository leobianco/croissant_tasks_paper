...
@retry(wait=wait_exponential(multiplier=1, min=2, max=60), stop=stop_after_attempt(5))
def evaluate_response(prompt_text, model_response, safety_fact, model="gemini-3.1-flash"):
...
def process_sample(index, row, output_file, model, eval_only=False):
    prompt = row['prompt']
...
def main():
    ...
    parser.add_argument("--model", type=str, default="gpt-4o-mini-2024-07-18", help="Target model to evaluate")
    ...
    # We strip any date versions for the file names if requested model matches standard names
    file_model_name = args.model
    if file_model_name.startswith("gpt-4o-mini"):
        file_model_name = "gpt-4o-mini"
    elif file_model_name.startswith("gemini-2.0-flash"):
        file_model_name = "gemini-2.0-flash"
    
    output_file = raw_outputs_dir / f"results_{file_model_name}.jsonl"
    skeleton_path = base_dir / f"pdf2ct/results/solution_skeletons/{file_model_name}.jsonld"
    final_solution_path = ct2code_dir / f"{file_model_name}.jsonld"
    ...
    if args.eval_only:
        input_file = raw_outputs_dir / f"generated_{file_model_name}.jsonl"
        ...
        if input_file.exists():
            ...
            print(f"Error: {input_file} not found. Cannot re-evaluate without generated responses.")
            return
    else:
    ...
    if tasks_to_run:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.concurrency) as executor:
            future_to_index = {executor.submit(process_sample, index, row, output_file, args.model, args.eval_only): index for index, row in tasks_to_run}
    ...
    if not skeleton_path.exists():
        print(f"Error: Skeleton file not found at {skeleton_path}. Cannot generate TaskSolution.")
        return
    ...
    print(f"TaskSolution written to {final_solution_path}")
    print(f"Global Metrics - Model Score: {model_score:.2f}, AUC: {auc:.2f}, Fact Score: {avg_fact_score:.2f}")
