...
- **Parent commit**: `02b87497` ("add - ct2code skill + updated pdf2ct skill").
...
Validator state on simpler inputs is healthier than yesterday: `tasks/testdata/valid_solution.jsonld` and `tasks/testdata/direct_task.jsonld` now PASS. So recent shape updates did land — just not on the two shapes our files exercise.
